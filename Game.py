...
#=== Conditions
    ...
    def isSpotFree(self, coords): #checks if the case is free to be taken by a player && coords contains two values (x,y)
        if(self._board[coords[1]][coords[0]] == 0):
            return True
...
#=== Functions
    ...
    def checkWinCond(self): #if Win Condition is verified, call setWinner function that also passes the self.isOver to True
        ...
        lDiagSum = self._board[0][0] + self._board[1][1] + self._board[2][2] #Diagonal starting top left
        rDiagSum = self._board[0][2] + self._board[1][1] + self._board[2][0] #Diagonal starting top right
        if(lDiagSum in winCond):
            ...
            for i in range(0,3):
                rowSum = sum(self._board[i])
                colSum = self._board[0][i] + self._board[1][i] + self._board[2][i]
    ...
    def updateSpot(self, coords): #update the board with the last spot picked and adapt to the actual player
        self._board[coords[1]][coords[0]] = 1 + (self.pTurn * -2) #pTurn = à 1 ou 0, on différencie les joueurs sur le plateau avec 1 et -1
...
if __name__ == "__main__":
    ...
    game = Game.Game(Player.Player('LnCol'), Player.Player('Json'))
